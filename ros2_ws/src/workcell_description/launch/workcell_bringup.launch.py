...
def generate_launch_description():
    ...
    # --- 2. Define file paths ---
    workcell_xacro_path = os.path.join(
        pkg_workcell_description, "urdf", "workcell.urdf.xacro"
    )
    rviz_config_path = os.path.join(pkg_workcell_description, "rviz", "workcell.rviz")
    ...
    try:
        robot_description_xml = xacro.process_file(
            ...
            mappings={
                ...
                "kortex_description": pkg_kortex_description,
            },
        ).toxml()
    except Exception as e:
        print("-------------------------------------------------")
        print(f"Xacro processing failed: {e}")
        print(f"Check your xacro file: {workcell_xacro_path}")
        print("-------------------------------------------------")
        raise e
    ...
    # This is your controller for the tables
    ...
    # This provides sliders for *only* the arm joints.
    ...
    return LaunchDescription(
        declared_arguments
        + [
            node_joint_state_publisher_gui,  # <-- FIX: Corrected variable name
            node_robot_state_publisher,
            # node_dual_table_controller,  # <-- FIX: Un-commented
            node_rviz,
        ]
    )
