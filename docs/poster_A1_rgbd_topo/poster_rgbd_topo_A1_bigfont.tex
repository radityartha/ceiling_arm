...
% ------------------------------------------------------------- knobs
\newcommand{\bodysize}{22}
\newcommand{\bodyskip}{27}
\newcommand{\pillsize}{30}
\def\figscale{0.92}
...
\vspace{1.5mm}
{\centering
\includegraphics[height=95mm]{GNG_static3.png}\hspace{5mm}
\includegraphics[height=95mm]{GNG_dynamic3.png}\par}
...
{\centering\includegraphics[trim=95 45 90 45, clip,
   width=\figscale\dimexpr0.86\linewidth\relax]{gng_capability.png}\par}
...
\begin{itemize}
  \item Rail travel, arm travel, tool distance, manipulability
  \item \textbf{Fit by OLS} on 168 measured picks ($R^2=0.857$):
        rail travel dominates
  \item \lead{Selection:} pool all 4 arms' nodes, score by $J$,
        \textbf{plan-only} round-robin
\end{itemize}

\psection{Experiments and Results}

\begin{itemize}
  \item \lead{Workspace gain:} rail active $\rightarrow$ \textbf{$4\times$}
...
\begin{itemize}
  \item \lead{Coverage:} \num{93.3\%} multi-arm vs.\ \num{63--65\%} fixed arm
  \item \lead{Energy:} \textbf{lowest mean} of multi-arm policies
        (\num{45.84~J} vs.\ 48.28~J nearest, $-5.1\%$)
  \item \lead{Trade-off:} more arm travel, less costly rail travel
\end{itemize}
...
\begin{itemize}[leftmargin=6mm, topsep=0pt]
  \item \textbf{One GNG, two layers}: environment + per-arm capability
  \item \textbf{Base-aware capability layer}: rail as a genuine DOF
  \item \textbf{Energy-aware cost $J$}: $\approx4\times$ workspace,
        93.3\% success, lowest energy, 35/35 picks
\end{itemize}
\end{keybox}
