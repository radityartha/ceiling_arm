...
\textcolor{red}{A secondary note on a diagnostic tool: the live \texttt{reachability\_cloud}
RViz coloring utility additionally gates on an 8-node ``enclosure'' check
(rejects a point that only has map nodes approaching from one side, a
boundary-safety heuristic pre-dating this experiment, documented in the
source). With that extra gate active, 2 of the 8 objects (cracker\_box,
scissors) read 0\% reachable at \emph{every} radius tested, even beyond
their true nearest-node distance, because they sit at a one-sided-density
fringe of this particular map. That gate is not part of the pipeline's real
reachability decision -- disabling it (a supported, pre-existing mode, not a
tuned parameter) recovers the 100\% result above -- but its blind spot is
itself a useful diagnostic for where this map's node sampling is
asymmetric, a direction for future map-density work. Full confusion-matrix
tables (with and without the enclosure gate), the nearest-node distance
computation, and raw logs are in \texttt{docs/experiment\_results.md} (E5)
and \texttt{docs/e5\_data\_2026-07-17/}.}
