...
manipulability and the effort needed to hold it against gravity, so \textcolor{red}{each node already fixes both an arm and a base rather than marking only whether a cell is reachable}. Treating the rail as a
...
\cite{Yoshikawa1985}. Therefore, a node must be considered more than simply 
...
placement, together with manipulability and a holding cost, rather than a single occupancy bit.
...
The inverse kinematics solver is maintained in a standard and classical form, 
with the capability map utilised to enumerate candidate 8-DOF configurations. 
Each configuration is characterised by its unique arm and rail placement, 
with the arm selection and base motion determined by the energy cost.
...
The robot consists of two rails mounted to the ceiling, side by side, with each rail 
supporting two arms. Each arm is a Kinova Gen3 Lite, a six-joint manipulator 
with a reach of approximately 0.76 m and a two-finger gripper at its 
wrist \cite{Kinova}, thus resulting in a total of four arms.  
It is evident that each rail contributes an additional degree of freedom 
to the pair of arms, resulting in a prismatic axis that enables horizontal 
movement of the carriage, and a revolute axis that facilitates rotation. 
Consequently, with respect to the ground, each arm establishes an 
8-DOF chain. The configuration of one arm is expressed as follows:
as $\mathbf{q} = [d,\ \theta,\ q_1,\ \ldots,\ q_6]$, where $d$ and $\theta$ are the translation 
and rotation of that arm's own rail, and $q_1 \ldots q_6$ the arm joints.  
The kinematic tree is anchored at a fixed world frame, thereby ensuring stability 
and reliability throughout the system. It is evident that each rail base is attached 
to the world, each arm base to its own rail, and a tool frame at the gripper defines 
the end-effector pose that the task is specified in. It is evident that the configuration 
of the system is such that the two arms are mounted on a single carriage. Consequently, 
any movement of the rail effects a simultaneous movement of both arms. This principle 
governs the relationship between the selection of an arm and the positioning of the base. 
In contrast, arms affixed to distinct rails exhibit independent movement. Fig.~\ref{fig:overview}
shows the platform and its frames alongside an overview of the end-to-end pipeline. 
This encompasses the processes of perception, energy-aware selection, planning 
and execution.

\subsection{Simulation Environment}

All experiments in this paper are conducted using NVIDIA Isaac Sim, a GPU-accelerated 
simulator with a physics engine that is well-suited for contact-rich 
manipulation \cite{IsaacSim2026,Mittal2023}. The simulated robot has been 
constructed from the same URDF description that defines the physical platform. 
This ensures that the kinematics and joint limits employed in the simulation 
are consistent with the real hardware. The scene is characterised by the placement 
of a set of everyday tabletop objects within the workspace. A number of these 
objects are positioned within easy reach of several arms, while at least one is 
deliberately placed near the edge of the workspace (Fig.~\ref{fig:scene}). 
The scene under consideration is observed by two RGB-D cameras placed 
above the workspace. These cameras constitute the system's sole exteroceptive 
sensor. It is evident that the images in question fulfil dual roles. 
The colour stream is utilised by the object detector, while the depth stream 
contributes to the construction of the collision octomap, which is employed for 
the purpose of planning. Consequently, a single sensing rig serves to facilitate 
both perception and collision avoidance. In this study, the simulator is utilised 
as the evaluation platform, with the physical robot being employed for validation 
...
The capability map of Section IV is constructed offline with the Pinocchio 
rigid-body dynamics library \cite{Carpentier2019}, which is employed for the 
purpose of forward kinematics when sampling configurations and for the gravity 
torque that imbues each map node with its holding cost. During the execution 
...
The Growing Neural Gas network model proposed by Fritzke \cite{Fritzke1995} 
was employed to analyse the provided dataset. Each node
...
$\mathbf{q}$ that produces it. The training is configured in such a manner that 
the search for the best-matching unit utilises the task part in isolation. This 
results in the nodes covering the reachable workspace, while adaptation updates 
the entire vector, thereby ensuring that each node's $\mathbf{q}$ converges to a 
configuration that is representative of its workspace cell. 
The formation of edges between co-activated nodes is attributable to 
competitive Hebbian learning, thereby endowing the map with a topology over 
the workspace. It can thus be concluded that each node is comprised of two 
constituent elements: firstly, a location that the end-effector is capable of 
attaining; and secondly, a representative 8-DOF configuration that achieves this 
location, including rail placement. This is the sense in which the map is 
base-aware. It is evident that since the rail translation and rotation are 
carried by $q$, the paths to a given point from disparate base placements result 
in different nodes. Consequently, a node's configuration remains a valid posture, 
rather than an average of incompatible base positions. In order to ensure the map 
remains faithful at the reachable boundary, where nodes would otherwise settle 
inward due to growth from interior samples alone, a shell of nodes is attached to 
the measured reach surface prior to the growth of the interior (Fig.~\ref{fig:gngmap}),
...
In addition to plain reachability, each node is associated with two quality values, 
which are utilised in the calculation of energy cost in Section V. 
The manipulability $w$ \cite{Yoshikawa1985} is indicative of the dexterity of 
the arm at a given node. Low values of $w$ are observed in the vicinity of the 
workspace edge and in positions that are singular, whilst high values of $w$ are 
observed in areas where the arm is able to move freely in any direction. 
The distinguishing characteristic that differentiates our map from a binary 
reachability grid, which merely indicates whether a cell is reachable or not, 
is this quality layer \cite{Zacharias2007}. The holding cost is defined as 
the norm of the gravity torque at the node's configuration. This measurement
quantifies the effort required to maintain the specified posture against
gravity. It is important to note that both values are computed 
once, offline, with the same Pinocchio model used for sampling 
\cite{Carpentier2019} and stored on the node. This ensures that 
they are available at query time without incurring any additional costs.

\section{Energy-Aware Arm and Base Selection}

In the context of the object pose from perception and the two capability maps, 
the selection stage determines the specific arm to utilise and the precise 
8-DOF configuration required to achieve the desired outcome. The decision-making 
process is characterised by the simultaneous consideration of both factors, 
with the energy cost being evaluated across a range of candidates derived from 
the available maps.
...
\hline
Gravity holding torque & $h$ & \textcolor{red}{0 (disabled)} & \textcolor{red}{2.04} \\
\hline
\end{tabular}
\label{tab:weights}
\end{center}
...
motion still needed to reach the object; and the holding term \textcolor{red}{is intended to represent the effort of fighting
gravity, though, as noted below, the calibration leaves it unweighted}. \textcolor{red}{The rail carries a Coulomb and viscous friction model
(mass- and coefficient-of-friction-derived, a stated modelling assumption rather
than measured hardware data; Section~VI-C), which is what lets rail-linear travel
dominate the fitted weights -- under a frictionless rail this same regression
gave a far weaker fit ($R^2=0.055$) and put nearly all weight on arm travel
instead. The gravity-holding term's OLS coefficient came out negative and weak,
so it is disabled ($w_{hold}=0$) rather than fit backwards.} We do not claim that
$J$ is the exact energy of a pick, but Section~VI
...
\begin{table}[htbp]
\caption{GNG capability map properties, per arm. \textcolor{red}{Updated 2026-07-16 for rail 2.0~m + ceiling filter; arm\_3/arm\_4 are $y$-mirrors of arm\_1/arm\_2 with matching properties.}}
\begin{center}
\begin{tabular}{|l|r|r|}
\hline
...
\hline
Surface points detected & \textcolor{red}{7560} & \textcolor{red}{7483} \\
\hline
Total nodes & \textcolor{red}{2915} & \textcolor{red}{2915} \\
\hline
Pinned boundary nodes & 600 & 600 \\
\hline
Interior nodes & \textcolor{red}{2315} & \textcolor{red}{2315} \\
\hline
Edges & \textcolor{red}{14{,}547} & \textcolor{red}{14{,}562} \\
\hline
Median node spacing (m) & \textcolor{red}{0.090} & \textcolor{red}{0.090} \\
\hline
Mean node spacing (m) & \textcolor{red}{0.092} & \textcolor{red}{0.092} \\
\hline
...
regression of measured energy on the six normalised $J$ terms (168 pooled picks),
