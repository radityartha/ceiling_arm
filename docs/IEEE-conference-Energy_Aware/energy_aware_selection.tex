...
the maintenance of order, and the delivery of items to the occupants \cite{IFR2025,Lisondra2026}.
...
One potential solution to this problem is to elevate the robot entirely off the floor \cite{Lalonde2025}.
...
\cite{DAGPlan2024,RoboPARA2026}. Their concern is the temporal and logical structure
...
\begin{minipage}{0.49\linewidth}\centering
  \includegraphics[width=\linewidth]{figure/RGBD_cam.png}\\
  {\footnotesize (a) camera~1}
\end{minipage}\hfill
\begin{minipage}{0.49\linewidth}\centering
  \includegraphics[width=\linewidth]{figure/RGBD2_cam.png}\\
  {\footnotesize (b) camera~2}
\end{minipage}
\caption{\textcolor{red}{The simulated workspace in NVIDIA Isaac Sim as seen by the two
overhead RGB-D cameras placed at the two ends of the corridor the rails run along:
(a) camera~1 and (b) camera~2. The tabletop objects used in the experiments are visible;
at least one object sits near the edge of the reachable workspace.}}
\label{fig:scene}
\end{figure}
...
saturated resolutions; 4.03$\times$ when both volumes are restricted below the ceiling)}.
...
grid sweep. \textbf{Result: 35/35 (100\%) pick success, 0 failures in every
requested category} (no-detection: 0, IK $-31$/unreachable: 0, no
collision-free plan: 0, unrecovered execution-abort: 0). Mean time-to-pick
...
objects (3864/3864 matched detections), mean localization error 0.019~m
(range 0.006--0.070~m per object across cameras), and the false-positive
...
``bottle''); no cross-object confusion was observed. A confidence-threshold
sensitivity check (conf$=0.10$, 150 frames) found detection rate and
localization error essentially unchanged -- already saturated at
conf$=0.25$ -- but false positives rose to 15.2\% (215/1415), confirming
conf$=0.25$ as a sound operating point. This result revises an earlier,
pessimistic finding about YOLOE on this simulator: that finding predated a
switch to the larger \texttt{11m} detector weights, and with the current
weights YOLOE is a viable perception input on this platform, not merely a
sim-only crutch. Full per-object table, the raw comparison script
...
all 4 arms) was compared against a voxel-based reachability classifier (the
same live \texttt{reachability\_cloud} coloring tool used for RViz
visualization: a point counts as reached if a GNG node lies within
\texttt{reach\_radius} \emph{and} an 8-node enclosure gate is satisfied),
swept over \texttt{reach\_radius}$\in\{0.08,0.12,0.16\}$~m plus the default
adaptive ($\approx$0.090~m). \textbf{Classification accuracy was identical
(75\%, 6/8) at every radius tested} -- the same two objects (cracker\_box,
scissors) were misclassified as unreachable at every setting, even though
their ground-truth nearest-node distance (0.12--0.15~m, computed directly
from the GNG map) falls within the largest radius tested. Diagnosis: the
enclosure gate, not raw distance, is the actual decision boundary for these
two fringe-positioned objects, so widening \texttt{reach\_radius} alone
cannot fix the misclassification. Critically, \textbf{this measures a
secondary RViz visualization heuristic, not the pipeline's real reachability
decision}: \texttt{gantry\_reach\_executor} calls per-arm IK directly on its
own candidate pre-grasp point rather than requiring full-surface enclosure,
which is exactly why both objects picked successfully in E6 despite this
tool's 0\% reading. The paper's actual selection pipeline (E3's ranking +
IK, end-to-end validated in E6) is unaffected and reached 100\% correct
reachability classification. Full confusion-matrix table, the nearest-node
distance diagnosis, and raw logs are in \texttt{docs/experiment\_results.md}
(E5) and \texttt{docs/e5\_data\_2026-07-17/}.}
