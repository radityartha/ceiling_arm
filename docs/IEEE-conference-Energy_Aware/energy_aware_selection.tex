...
\end{minipage}
\caption{\textcolor{red}{Overview of the dual-gantry quad-arm ceiling robot and the
proposed pipeline. (a) The physical ceiling-mounted platform in our trailer living
laboratory -- the hardware that motivates this work; \emph{all experiments in this
paper are in simulation}, hardware validation is future work (Section~VII). (b) The
same platform modeled as its Isaac Sim digital twin: two rails mounted to the
ceiling, each carrying two Kinova Gen3 Lite arms and each adding a prismatic axis
($d$) and a revolute axis ($\theta$) that its two arms share, so each arm forms an
8-DOF chain $\mathbf{q}=[d,\theta,q_1 \ldots q_6]$; two overhead RGB-D cameras
observe the workspace. (c) The end-to-end pipeline, from open-vocabulary
perception, through energy-aware arm-and-base selection over the base-aware
capability map, to collision-free planning and execution.}}
\label{fig:overview}
\end{figure*}
...
\textcolor{red}{This confirms the reachable-workspace gain from treating the rail
as a DOF and the boundary-seeding accuracy already reported in Section~I
(1.920~m$^3$ locked vs.\ 7.899~m$^3$ active at 0.05~m voxel resolution; edge
shortfall 0.221~m~$\to$~0.008~m with boundary seeding); see
...
\caption{\textcolor{red}{Distribution of measured trajectory energy per multi-arm
selection policy under the rail-friction model ($\diamond$~=~mean); energy-aware
selection attains the lowest mean, though the distributions overlap heavily -- a
