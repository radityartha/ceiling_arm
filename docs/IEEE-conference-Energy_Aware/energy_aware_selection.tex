...
arm and base choice; \textcolor{red}{over a 60-position grid, the multi-arm policies
(energy-aware, nearest, random) each reach 93.3\% plan success, versus 60--65\% for
any single fixed arm, and energy-aware selection substantially lowers both summed
arm-joint travel (6.55~rad mean vs.\ 7.94--8.13~rad for nearest/random) and
rail-rotation travel (0.68~rad vs.\ 1.37~rad for nearest/random, roughly half),
without trading one for the other, though aggregate measured mechanical
energy across the grid is comparable across the three multi-arm policies and not
uniformly lower under this idealised (damping-free) dynamics model}.
\end{abstract}
...
plan success from 60--65\% (any single fixed arm) to 93.3\%, and energy-aware
selection specifically reduces both summed arm-joint travel and rail-rotation
travel relative to the nearest
and random baselines \textcolor{red}{(arm: 6.55~rad mean vs.\ 7.94/8.13~rad;
rail-rotation: 0.68~rad vs.\ 1.37/1.37~rad, roughly half; aggregate
measured mechanical energy across the grid was comparable across the three
multi-arm policies rather than uniformly lower for energy-aware selection, a
limitation of the idealised damping-free dynamics model used to compute it --
see Section~VI-C)}. In order to ground these findings, our simulation in NVIDIA Isaac Sim \cite{IsaacSim2026}
...
eight-degree-of-freedom chain. The configuration of one arm is expressed as follows:
...
The scene is observed by two RGB-D cameras placed above the
...
freedom. We build one such capability map per arm, so the \textcolor{red}{four arms across the two rails
are described by four maps} that we query independently in Section~V.
...
and traj\_energy per selection mode, data ready (Table~\ref{tab:e3},
\texttt{/tmp/e3\_\{energy,nearest,random,fixed\_arm1..4\}.csv})}
...
\begin{table}[htbp]
\caption{\textcolor{red}{E3: selection-policy comparison, 60 target poses,
plan-only (mean / median over successful picks).}}
\begin{center}
\resizebox{\linewidth}{!}{%
...
\hline
\textcolor{red}{energy (ours)} & \textcolor{red}{56/60 (93.3\%)} & \textcolor{red}{1.14/1.16} & \textcolor{red}{0.68/0.42} & \textcolor{red}{6.55/5.89} & \textcolor{red}{13.19/12.31} \\
\hline
\textcolor{red}{nearest} & \textcolor{red}{56/60 (93.3\%)} & \textcolor{red}{1.19/1.41} & \textcolor{red}{1.37/1.33} & \textcolor{red}{7.94/7.50} & \textcolor{red}{12.01/10.72} \\
\hline
\textcolor{red}{random} & \textcolor{red}{56/60 (93.3\%)} & \textcolor{red}{1.17/1.28} & \textcolor{red}{1.37/1.35} & \textcolor{red}{8.13/7.69} & \textcolor{red}{12.87/11.66} \\
\hline
\textcolor{red}{fixed (best: arm\_4)} & \textcolor{red}{39/60 (65.0\%)} & \textcolor{red}{1.14/1.21} & \textcolor{red}{1.22/1.23} & \textcolor{red}{7.47/7.17} & \textcolor{red}{12.87/11.19} \\
\hline
\textcolor{red}{fixed (worst: arm\_3)} & \textcolor{red}{36/60 (60.0\%)} & \textcolor{red}{1.19/1.41} & \textcolor{red}{1.65/1.44} & \textcolor{red}{7.77/7.35} & \textcolor{red}{11.50/9.87} \\
\hline
\end{tabular}}
\label{tab:e3}
\end{center}
...
\textcolor{red}{The four single-arm \texttt{fixed} baselines cluster at 60--65\%
success and are roughly symmetric between gantries (arm\_1 37/60 vs.\ its
$y$-mirror arm\_3 36/60; arm\_2 38/60 vs.\ arm\_4 39/60), consistent with the
workcell's near-mirror geometry. All three multi-arm policies reach 93.3\%, about
30 points above any single fixed arm, showing that four-arm coverage is the
dominant reachability gain. Energy-aware selection's distinct, defensible
contribution in this dataset is a substantial reduction in travel on \emph{both}
axes at once, not a trade of one for the other: summed arm-joint travel
(6.55~rad mean vs.\ 7.94/8.13~rad for nearest/random) and rail-rotation travel
(0.68~rad vs.\ 1.37/1.37~rad for nearest/random, roughly half, Table~\ref{tab:e3}),
with rail-linear travel essentially unchanged; its aggregate measured energy (13.19/12.31~J
mean/median) is, honestly, \emph{not} the lowest of the three multi-arm
policies (nearest: 12.01/10.72~J; random: 12.87/11.66~J) on this grid, which we
attribute to $J$'s known weak correlation with the idealised (damping-free)
Pinocchio energy model (the URDF has no joint damping/friction and both rail axes
are orthogonal to gravity, so this energy likely under-represents real motor
draw) rather than a failure of the selection method. Per-arm
win counts confirm all four arms are actually selected across the grid, including
gantry\_2's arm\_3/arm\_4 (energy: arm\_2 18, arm\_3 14, arm\_1 12, arm\_4 12).
In 30 of 56 comparable positions, energy and nearest selected a \emph{different}
arm; in a representative case (target $x{=}0.00,y{=}-0.60,z{=}1.15$) energy chose
arm\_3 (0.574~m rail-linear travel, 5.39~rad arm travel, measured energy 11.36~J)
over nearest's arm\_2 (0.000~m rail travel, 12.22~rad arm travel, 14.63~J) --
accepting more base travel to avoid a much larger arm-joint excursion, for both a
lower cost $J$ and a lower realised energy. This is the case plotted in
...
% \textcolor{red}{UNVERIFIED -- LIKELY FABRICATED. This exact title appears in NO source; pp. 3841--3846 belong to a different ICRA paper; the DOI resolves to a real IEEE doc (1014387) whose true title could not be confirmed (Xplore JS-blocked). DO NOT use as-is: verify IEEE document 1014387 directly, or replace this cite -- e.g. with the already-verified ceiling-manipulator anchor Jayathilaka2025.}
